...
var serverUrl = "/zdpyc/door" + "/static/js/jhelper/";
  ...
  toolbar.Function.textTips = {
    ...
    getTextNode: function (element) {
      ...
      for (var i = 0; i < childNodes.length; i++) {
        ...
        var  isHaveGetMessageClaSS=$(thisChild).hasClass("getmessage");
        console.log("isHaveGetMessageClaSS:"+isHaveGetMessageClaSS);
        if(!isHaveGetMessageClaSS){
         ...
         if (thisChild.data == "下一步" ||thisChild.data == "保存" ||thisChild.data == "上一步" ||thisChild.data == "提交"  ) {

                 console.log("yes");
          }else if (thisChild.nodeName == "IMG" || thisChild.nodeName == "INPUT" || thisChild.nodeName == "OBJECT" || thisChild.nodeName == "SELECT") {
            this.allTextNode.push(thisChild);
          }
        }
      }
    ...
    AddTag: function () {
      ...
      for (var a = 0; a < this.allTextNode.length; a++) {
        ...
        if (this.allTextNode[a].data == "下一步" ||this.allTextNode[a].data == "保存" ||this.allTextNode[a].data == "上一步" ||this.allTextNode[a].data == "提交"  ) {

        }else{
          if (this.allTextNode[a].nodeName == "SVG" ||this.allTextNode[a].nodeName == "BUTTON" ) {

          } else if (this.allTextNode[a].nodeName == "IMG" ) {
            tagNode.setAttribute("class", "getmessage");
            var newNode = this.allTextNode[a].cloneNode(true);
            tagNode.appendChild(newNode);
          } else if ( this.allTextNode[a].nodeName == "INPUT" || this.allTextNode[a].nodeName == "SELECT") {
            /* tagNode.setAttribute("class", "getmessage");
             var newNode = this.allTextNode[a].cloneNode(true);
             tagNode.appendChild(newNode);*/
          } else if (this.allTextNode[a].nodeName == "OBJECT") {
            if(this.allTextNode[a].parentNode){
              if(this.allTextNode[a].parentNode.nodeName != "OBJECT"){
                tagNode.setAttribute("class", "getmessage");
                var newNode = this.allTextNode[a].cloneNode(true);
                tagNode.appendChild(newNode);
              }
            ...
            if (true) {//reg.exec(newString) == null) {
              tagNode.setAttribute("class", "getmessage");
              var  isHaveGetMessageClaSS=$(tagNode).hasClass("getmessage");
              console.log("isHaveGetMessageClaSS:"+isHaveGetMessageClaSS);
              if (this.allTextNode[a].parentNode != null) {
      ...
      for (var b = 0; b < this.allTextNode.length; b++) {
        if (this.allTextNode[b].data == "下一步" ||this.allTextNode[b].data == "保存" ||this.allTextNode[b].data == "上一步" ||this.allTextNode[b].data == "提交"  ) {

        }else
        if (this.allTextNode[b].nodeName == "SVG" || this.allTextNode[b].nodeName == "INPUT" || this.allTextNode[b].nodeName == "SELECT") {
          /* tagNode.setAttribute("class", "getmessage");
           var newNode = this.allTextNode[a].cloneNode(true);
           tagNode.appendChild(newNode);*/
        }else{
    ...
    getText: function (element) {
      ...
      var textMessage = "";
       if (ele.firstChild.nodeName == "IMG") {
        ...
        switch (inputType) {
          case "button":
            if(ele.firstChild.getAttribute("value")){
              textMessage = "表单按钮：" + ele.firstChild.getAttribute("value");
            }
            break;
          case "image":
            if(ele.firstChild.getAttribute("alt")){
          ...
          case "reset":
            ...
            break;
          case "file":
            if(ele.firstChild.getAttribute("title")){
    ...
    mainMethod: function () {
      ...
      if (!this.textGetState) {
        if($(".ty-button-oprator")){
          $(".ty-button-oprator").css("bottom","160px");
        }
        if ($("#gettextmessagebox").length > 0) {
          //note 说明已经存在，只是给隐藏了
          this.textGetState = true;
          document.body.style.paddingBottom = 160 + "px";
          $(mainBoxID).css("marginBottom","160px");
          ...
          // document.body.insertAfter(newMessageBox, $(mainBoxID).get(0));
          $(newMessageBox).insertAfter($(mainBoxID).get(0));
          $(mainBoxID).css("marginBottom","160px");
          document.body.style.paddingBottom = 160 + "px";
       ...
       bindMessageBoxJS();
        ...
        if($(".ty-button-oprator")){
          $(".ty-button-oprator").css("bottom","0px");
        }
        $("#gettextmessagebox").hide();
        $(mainBoxID).css("marginBottom","0px");
...
function readPage () {
  toolbar.Function.textTips.readConfig(!toolbar.isRead);
  var dom = document.getElementById('readPageSwitch');
  if (toolbar.isRead) {
    // dom.innerHTML = '关闭语音'
    dom.title = '关闭语音';
    dom.className = 'closeReadPage';
  } else {
    // dom.innerHTML = '开启语音'
    dom.title = '开启语音';
    dom.className = 'openReadPage';
  }
}
function openMsgBox () {
  openMsgBoxFlag = 1;
...
function closeMethod(){
  if (toolbar.Function.textTips.textGetState) {
    toolbar.Function.textTips.firstRun = false;
    toolbar.Function.textTips.mainMethod();
  }
}

//note 返回是否打开屏幕
function backIsOpenFlag(){
...
function reOpenMethod(){
  if (INSTANCE.openFlag == "1") {
    if (toolbar.Function.textTips.textGetState) {
      toolbar.Function.textTips.firstRun=false;
      toolbar.Function.textTips.getTextNode($(mainBoxID).get(0));
      toolbar.Function.textTips.reInitMainMethod();
    }
  }
...
function setTopImgHeight(){
  if (INSTANCE.openFlag == "1") {
     document.getElementsByClassName('home-xian-index-header-img')[0].style.setProperty('top',  "36px", "important");
  }
};
function setTopHeadImgHeight(){
  if (INSTANCE.openFlag == "1") {
    document.getElementsByClassName('street-header-img')[0].style.setProperty('top',  "150px", "");
  }
}

//note 重置配色
function reSetColor(code){
  $(".disabilitiesN2  li").removeClass("disabilitiesNow");
  $(".jhelper_btn01").addClass("disabilitiesNow");
  asySetColorFunction('0');
}

//note 重置 當前的配色
function reSetColorCurrent(){

  asySetColorFunction(INSTANCE.colorFlag);
}

//note 设置顶部距离
function setTopHeight(){
  if (INSTANCE.openFlag == "1") {
    setTimeout("setTopImgHeight()",1500);
    debugger;
    if(document.getElementsByClassName('street-header-img')[0]){
      setTimeout("setTopHeadImgHeight()",500);
    }
  }
};
...
function initData() {
  ...
  if (!INSTANCE.openFlag) {
    INSTANCE.openFlag = "0";
    localStorage.setItem("isOpenFlag","0");
  }
...
function toggleToolBar() {
  ...
  if (INSTANCE.openFlag == "0") {
    openToolBar();
    INSTANCE.openFlag = "1";
    $(".userPop").addClass("userPop95").removeClass("userPop");
    SetCookie("openFlag", "1");
  } else {
    INSTANCE.openFlag = "0";
    $(".userPop95").addClass("userPop").removeClass("userPop95");
...
function openToolBar() {
  ...
  wcagNavDiv.show(1000, function () {
    ...
    // document.body.style.paddingTop = wcagNavDiv.get(0).offsetHeight + "px"+" !important";
    if(document.getElementsByClassName('home-xian-index-header-img')[0]){
      setTimeout("setTopImgHeight()",500);
    }
    if(document.getElementsByClassName('street-header-img')[0]){
      setTimeout("setTopHeadImgHeight()",500);
    }
...
function creatWcagNav() {
  ...
  $(navToolbarDiv).insertBefore(document.body.firstChild);

  /*  var navToolbarDiv1 = document.createElement("div");
    navToolbarDiv1.setAttribute("id", "myHelp");
    navToolbarDiv1.style.display = "block";
    navToolbarDiv1.innerHTML = "testaaa";
    $(navToolbarDiv1).insertAfter(document.body.firstChild);*/


//  document.body.insertBefore(navToolbarDiv, document.body.firstChild);
//  insert​After(navToolbarDiv,document.body.firstChild);
  // 绑定页面效果脚本
  bindPageJS();
}
...
function getToolbarInnerText() {
  ...
  textHTML += "</ul>";
  textHTML += "<ul class=\"disabilitiesN1\">";
  textHTML += "<span id=\"txtName2\">&nbsp;语音</span>";
  textHTML += "<li class=\"jhelper_btn01\"><button  title=\"开启语音\" id='readPageSwitch' onclick=\"readPage();\"class=\"openReadPage\"></button></li>";
  // textHTML += `<li class="jhelper_btn01">
  //               <button title="开启语音" id="readPageSwitch" onclick="readPage()" class="openReadPage"></button>
  //             </li>`;
  // textHTML += `<li class="jhelper_btn02"></li>`;
  textHTML += "</ul>";
...
function openTextFunction() {
  if (INSTANCE.textFlag == "0") {
    ...
    $("body").css("paddingLeft","100px");
    if($(".seamless-scroll-new")){
      $(".seamless-scroll-new").hide();
    }
  } else {
    if($(".seamless-scroll-new")){
      $(".seamless-scroll-new").hide();
...
function changeStyle() {
  ...
  for (var i = 0; i < linkStyle.length; i++) {
    if (linkStyle[i].getAttribute("href") != "./static/js/jhelper/jhelper_tool_style.css"
      && linkStyle[i].getAttribute("href") != "./static/js/jhelper/newlib/css/jhelper_no_clear.css") {
      linkStyle[i].setAttribute("href", textStyleCSS);
    }
  }
  var styleTags = document.getElementsByTagName("style");
  while(styleTags.length){
...
function openPageFunction(flag) {
  if(document.getElementsByClassName('home-xian-index-header-img')[0]){
    document.getElementsByClassName('home-xian-index-header-img')[0].style.setProperty('top', "0px", "important");
  }
  ...
  if (flag == "0") {
    if(INSTANCE.zoomValue>1.5){
      return;
    }


    INSTANCE.zoomValue = INSTANCE.zoomValue + 0.1;
...
//note 定义切换菜单要修改的颜色
var isMenuInto=false;//note 如果是true，那么就重新刷一遍，否则不刷
function updateMenuColor(){
  setTimeout(asySetColorFunction(), 3000);
}
//note 判断是否打开无障碍
function getIsOpenColor(){
  if (INSTANCE.openFlag == "0") {
     return false; //note 代表还没有打开无障碍，不处理，跳出去
  } else {
    //note 代表已经打开了打开无障碍，继续处理
      return INSTANCE.colorFlag;
  }
}
function asySetColorFunction(mode) {
  localStorage.setItem("isOpenFlag",mode);
  if (INSTANCE.openFlag == "0") {
    return; //note 代表还没有打开无障碍，不处理，跳出去
  } else {
    //note 代表已经打开了打开无障碍，继续处理
  ...
  if(!mode){
    mode=INSTANCE.colorFlag;
  }
  asyDoSetColorFunction(mode);
  setTimeout("asyDoSetColorFunction("+mode+")",2000)
}

function changeColorStyle(bgColor, bgImage, color) {
  $(mainBoxID).find("*:not(#wcagnav,.gotop)").css({
  ...
  if($(".gotop")){
    $(".gotop").css("background-image","url(data:image/png;base64,) no-repeat center");
  }
  if($(".prev")){
    $(".prev").css("background-image","url(data:image/svg+xml;base64,)");
  }
  if($(".next")){
    $(".next").css("background-image","url(data:image/svg+xml;base64,)");
  }
...
function createNode(mainDiv, title) {
  ...
  // mainDiv.parentNode.insertAfter(newNode, mainDiv);
  $(newNode).insertAfter(mainDiv);
