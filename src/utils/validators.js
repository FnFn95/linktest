...
const Validate = {
    ...
    validator: (rule, value, callback) => {
      if (value) {
        ...
        if (value.match(rule.regexp)) {
          ...
          callback(new Error(rule.message || 'IC卡号格式不正确'));
        }
      } else {
        callback();
      }
  ...
  identification: {
    ...
    regexp: /^[\d()（）a-zA-Z]*$/,
    validator: (rule, value, callback) => {
      //note 新的校验规则 3048 小于18位，支持数字，字母，括号
      if (value) {
        // if (value.length != 18) {
        ...
        // }
        if (value.length <= 18) {
          if (value.match(rule.regexp || Validate.identification.regexp)) {
            callback();
          } else {
            callback(new Error(rule.message || '请输入正确的身份证号！'));
          }
        } else {
          callback(new Error(rule.message || '身份证号码应不超过18位！'));
        }
      } else {
    ...
    validator: (rule, value, callback) => {
      if (value) {
        if(!isNaN(value)&&value>0){
          ...
          callback(new Error(rule.message || '请输入正数'));
        }
        // if (value.match(rule.regexp || Validate.decimalNumber.regexp)) {
        //   callback();
        // } else {
        //   callback(new Error(rule.message || '请输入正数'));
        // }
